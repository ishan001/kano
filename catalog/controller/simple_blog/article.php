	...
	class ControllerSimpleBlogArticle extends Controller {
		public function index() {
			...
			if (isset($this->request->get['limit'])) {
				...
				$limit = (int)$this->config->get('simple_blog_root_posts_per_page');
			}
      
		      if (!$limit)
		        $limit = 20;
      
		      $description_length = (int)$this->config->get('simple_blog_post_desc_length');
    		...
    		$this->response->setOutput($this->load->view('simple_blog/article.tpl', $data));	
		}
		...
		public function view() {
          ...
          if ($article_info['avatar'])
            $data['author_avatar'] = HTTP_SERVER . 'image/' . $article_info['avatar'];
          else
            $data['author_avatar'] = '';  
          ...
          $data['button_wishlist'] = $this->language->get('button_wishlist');
		      $data['button_compare'] = $this->language->get('button_compare');
					...
					foreach($related_product as $product) {
						...
						$image = ($product_info['image']) ? $this->model_tool_image->resize($product_info['image'], 250, 250) : false;
        		...
        		$this->response->setOutput($this->load->view('simple_blog/article_info.tpl', $data));	
		...
		public function comment() {
         ...
         $this->response->setOutput($this->load->view('simple_blog/article_comment.tpl', $data));
		}
